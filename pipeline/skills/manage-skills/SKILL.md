...
9. **파이프라인 에이전트 정의** — `pipeline/agents/*.md`는 에이전트 지침이며 애플리케이션 코드가 아님
10. **파이프라인 스킬/스택 가이드라인** — `pipeline/skills/*.md`, `pipeline/stacks/**/*.md`는 행동 가이드라인
11. **파이프라인 템플릿** — `pipeline/templates/`는 스캐폴딩 템플릿
12. **생성된 문서** — `docs/errors/`, `docs/qa-logs/`, `docs/pipeline-logs/`는 Agent 산출물
