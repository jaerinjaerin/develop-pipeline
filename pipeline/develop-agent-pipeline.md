...
| 08 | 배포 Agent | Docker 이미지 빌드 → Staging 자동 배포 + Health Check → Production 승인 배포 | 자동+**수동** |

**결과** — 퍼블리싱 HTML → 실제 프로젝트 코드 (FE 컴포넌트 + BE API + DB 스키마 + E2E 테스트 + 문서) 완성 → Staging/Production 배포. 사람은 문서 검토 + Production 배포 승인만 하면 됩니다.
...
QA 전체 통과 후 Docker 기반으로 Staging 자동 배포 및 Production 승인 배포를 수행. 배포 결과를 문서화하고 Notion에 기록합니다.

`Docker 빌드` `Staging 자동 배포` `Health Check` `Production 승인 배포` `롤백` `File System MCP` `Notion MCP`

#### EXAMPLE · 로그인 페이지 QA 통과 → 배포 과정

**`STEP 1`** 사전 검증 + Docker 이미지 빌드

```
Docker 설치 확인 → deploy/ 설정 파일 4개 확인
→ FE 이미지 빌드: shopping-mall-frontend:20260220-a1b2c3d
→ BE 이미지 빌드: shopping-mall-backend:20260220-a1b2c3d
```

**`STEP 2`** Staging 자동 배포 + Health Check

```
docker-compose --profile staging up -d
→ FE: GET http://staging.example.com/ → 200 OK (142ms)
→ BE: GET http://staging.example.com/api/health → 200 OK (38ms)
→ Staging 배포 성공
```

**`STEP 3`** ⏸️ Production 승인 대기

```
"Staging 배포 완료. Production 배포를 진행하시겠습니까?"
→ 사람 승인 → Production 배포 + Health Check
→ 배포 로그 문서 생성 (docs/deploy-logs/)
→ Notion PIPELINE_LOG 기록
```

| Staging 배포 실패 시 | 롤백 3회 실패 시 |
|---|---|
| 이전 성공 이미지로 롤백 | 자동 복구 불가 판단 |
| Health Check 재실행 | 1~3회차 시도 이력 통합 요약 |
| 롤백 성공 → 에러 문서화 | ⏸️ 파이프라인 일시 정지 + 사람 에스컬레이션 |

↓

### 최종 결과

| QA 실패 시 | QA 성공 시 | 배포 실패 시 |
|---|---|---|
| 🔄 해당 Agent 재실행 | ✅ Staging → Production 배포 완료 | 🔄 롤백 → 3회 실패 시 에스컬레이션 |
| Orchestrator가 FE/BE 판단 후 해당 Agent에게 재할당 | 다음 페이지 파이프라인 시작 | Agent 08 롤백 절차 자동 실행 |
...
| **08 · 배포** | Agent 07 성공 로그 + deploy/ 설정 파일 | docs/deploy-logs/{날짜}\_{환경}.md + Notion PIPELINE_LOG 페이지 |
...
| 트리거 | → | 액션 |
|---|---|---|
| QA 전체 통과 | → | **Agent 07 실행** → 성공 로그 문서화 → **Agent 08 Staging 자동 배포** |
| Staging 배포 성공 | → | **⏸️ Production 승인 대기** → 승인 시 Production 배포 |
| 배포 실패 | → | **Agent 08 롤백 실행** → 롤백 성공 시 에러 문서화 |
| 롤백 3회 실패 | → | 자동 복구 불가 판단 → **⏸️ 사람에게 에스컬레이션** → 파이프라인 일시 정지 |
...
| 08 · 배포 | `File System MCP` `Notion MCP` (선택적) |
...
project/
  ...
  docs/                   ← Agent가 생성하는 문서
    ...
    deploy-logs/          ← Agent 08 배포 로그 결과물
```
